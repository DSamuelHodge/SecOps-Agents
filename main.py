...
from tools.packet_analyzer import PacketAnalyzer, run_network_analysis
...
def handle_result(message) -> None:
    print(f"SecOps Result: {message.data}")

human_consumer = CallableMessageConsumer(handler=handle_result, message_type="human")

# Create and launch the system
launcher = ServerLauncher(
    agents, control_plane, message_queue, additional_consumers=[human_consumer]
)

if __name__ == "__main__":
    launcher.launch_servers()

    # print("SecOps Llama Agents system is running.")
    # print("Use the following command in another terminal to interact with the system:")
    # print("llama-agents monitor --control-plane-url http://127.0.0.1:8000")