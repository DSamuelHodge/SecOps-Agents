...
class SuricataSC:
    ...
    def send_command(self, command, arguments=None):
        if command not in self.cmd_list and command != "command-list":
            raise SuricataCommandException(
                "L{}: Command not found: {}".format(get_linenumber(), command)
            )
        ...
        if ready[0]:
            ...
            cmdret = None
        if not cmdret:
            raise SuricataReturnException(
                "L{}: Unable to get message from server".format(get_linenumber)
            )
    ...
    def connect(self):
        ...
        if cmdret["return"] == "NOK":
            raise SuricataReturnException(
                "L{}: Error: {}".format(get_linenumber(), cmdret["message"])
            )
        ...
        for c, spec in enumerate(cmd_specs, 1):
            ...
            if spec["required"]:
                ...
                try:
                    ...
                    phrase = (
                        " at least" if required_args_count != len(cmd_specs) else ""
                    )
                    msg = "Missing arguments: expected{} {}".format(
                        phrase, required_args_count
                    )
                    raise SuricataCommandException(
                        "L{}: {}".format(get_linenumber(), msg)
                    )
                except ValueError as ve:
                    raise SuricataCommandException(
                        "L{}: Erroneous arguments: {}".format(get_linenumber(), ve)
                    )
            elif c < len(full_cmd):
                arguments[spec["name"]] = spec_type(full_cmd[c])
    ...
    def parse_command(self, command):
        ...
        if cmd in self.cmd_list:
            ...
            raise SuricataCommandException(
                "L{}: Unknown command: {}".format(get_linenumber(), command)
            )
        return cmd, arguments
